...
fn monitor_for_changes_and_rebuild(
    ...
    thread::spawn(move || {
        ...
        fn event_triggers_rebuild(
            ...
            paths_to_watch: &Vec<(PathBuf, PathKind, ShouldTriggerRebuild)>,
        ) -> bool {
            match event {
        ...
        'outer: while let Ok(event) = rx.recv() {
            ...
            trace!(
                "Starting build in {}.{:0>3}s if no more changes detected",
                event_timeout.as_secs(),
                event_timeout.subsec_nanos() / 1_000_000
            );
            ...
            while Instant::now() < deadline {
                match rx.recv_timeout(deadline - Instant::now()) {
                    Ok(event) => {
                        ...
                        trace!(
                            "Noticed follow-up change; waiting additional {}.{:0>3}s for more",
                            event_timeout.as_secs(),
                            event_timeout.subsec_nanos() / 1_000_000
                        );
